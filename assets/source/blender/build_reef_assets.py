...
Run from the repository root with Blender 5.1:
    ...
    & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" `
        ...
        --python assets/source/blender/build_reef_assets.py

The sources in ``assets/source/meshmaker/`` are MeshMaker output and are opened read-only.
...
VAT_OUTPUT_DIR = os.path.join(PROJECT_DIR, "assets", "runtime", "vat")
...
def build_coral(filename: str, node_name: str, target_triangles: int) -> dict:
    ...
    path = os.path.join(
        MODEL_OUTPUT_DIR,
        node_name.lower().replace("coral", "coral_") + ".glb")
...
def build_small_fish() -> dict:
    ...
    result = build_vat.bake(
        ...
        fps=FPS,
        output_base=os.path.join(VAT_OUTPUT_DIR, "small_fish_vat"),
