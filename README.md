...
Press **E** at the colony ship and the city panel opens with one button on it. **RELEASE SETTLERS** puts six Meeps on the grass — white spheres for now — and they get to work: they fell the trees around the lander for biomass, spend it raising a cloner, and once the cloner is up they walk into it and come out as more of themselves, which buys the town more hands to cut more trees. Everything under that button is built to end up as a planet of towns rather than as one town, so nothing here knows where Vacationer's Landing is: a second lander dropped anywhere gets its own colony by carrying its own `colony_site` name.

| Script | Holds |
| --- | --- |
| `game/meeps/meep_colonies.gd` | The registry under `Planet`: founding a site, releasing a wave, and the joiner snapshot |
...
| `game/meeps/meep_grid.gd` | 2 m cells over 256 m, classified off the height field as walkable, steep, water or void |
| `game/meeps/meep_claim.gd` | The boundary: a flood fill from the ship that stops where the ground does |
...
| `ui/city/city_menu.gd` | The panel: the button, the settler count, the biomass bank and what the town is doing |

**A Meep is a row, not a node.** A settler is one index across a dozen packed arrays and every one of them is drawn by a single `MultiMesh`, which is the bargain `GroundCover` makes for three hundred thousand plants rather than the one `FaunaMob` makes for fourteen creatures. The end state is what decides that: a world of towns growing at once, most with nobody watching, none of them costing a node each. What it costs is physics — a row cannot be a `CharacterBody3D` — so Meeps walk on the analytical height field and keep out of holes by consulting a grid baked from it. On this planet that is barely a compromise, because terrain colliders only exist within a couple of hundred metres of a viewer and a body would be walking on nothing most of the time anyway. What it does not cost is being able to point at one: eight `StaticBody3D` proxies follow the nearest Meeps, the same trick `FaunaSpawner` uses for its light pool, so the interaction ray finds a real collider on whichever one you walked up to.

**The boundary is derived, not authored.** The claim is a breadth-first fill from the ship capped at 100 m, and it stops when the cells do: the chasm beside the lander and the shoreline 70 m out are not special cases, they are cells the fill would not cross. That is what makes it work at a site nobody has surveyed, which is the whole point, since the next colony is wherever a Meep expedition decides. The wall is one instance per border cell edge, so a town that grows is the same mesh with more instances in it.

**Distance decides how often a Meep thinks, not whether it does.** Inside 120 m of a player they step ten times a second and read their ground from the field at the spacing the terrain is drawn at; out to 400 m it is a quarter of that off the grid's cached heights; past it a tenth, and they are not drawn at all — but they are still walking, still working, still finishing what they started, because the simulation is deliberately not tied to the renderer. A colony also only ever steps 256 Meeps per tick through a rotating cursor, so a crowded town loses reaction time instead of frames.

**Clients rebuild the town rather than being sent it.** The height field is identical on every peer, so a client bakes its own grid, fills its own claim and raises its own wall from the founding facts alone. What travels is the part that is genuinely a decision: where each Meep is, ten times a second, for the ones somebody is near, with the population, the bank and a byte of state per settler riding along in the same packet — the state because a client that did not know which Meeps are inside the cloner would draw them standing in the wall of it. Where a building **is** cannot be recomputed that way, because it was a choice, so placements go reliably and once; how far along it is rides the frequent packet with everything else, since a wrong construction fraction corrects itself a tenth of a second later.

**The colony takes hits for everyone in it.** It joins the `combatants` group as one node reporting `Faction.PLAYER`, then sorts out who was actually in the blast itself with `reaches` and `damage_at` per Meep — the same arrangement `GroundCover` uses to take a hit for three hundred thousand plants. It reports the whole town as its combat bounds so the shared hit is not scaled down against a centre nobody was standing at, and reports that centre **above the ground** rather than at the site map's zero, which is sea level: a town on a hillside that reported the latter would be a colony sitting inside a hill, out of range of hits that landed in the street and permanently behind cover.

**The economy is one number, and it comes out of the forest.** A Meep sent to mine walks to the nearest standing tree it can reach and cuts for six seconds, and the tree comes down through the same `DamageHit` the player's meteor uses, which is what makes a felled trunk fall over on every client without the colony knowing anything about how flora replicates. What it pays is the tree's own business — `FlowerTreeField.harvest_value` scales with real height, so a sapling is not worth the walk and a sixteen-metre trunk is — and it lands in the bank the moment the trunk does. Trees are taken nearest-first from the ship, so a town eats its way outward and the clearing you can see around the lander is the ledger.

**Which means the landing site has a ceiling, and it is a low one on purpose.** The colony asks every flora field in the damage group whether it grows `standing_near` and `harvest_value`, and takes whatever answers; at Vacationer's Landing exactly one does, the flower-tree colony, which is fourteen trees inside the town's grid and a few hundred biomass in total. That is enough for a cloner, a few huts and some settlers, and then the town runs out of forest and idles — which is the honest state of the economy rather than a bug in it. It is a deliberate choice about **what is safe to harvest**: the streamed cover fields hold hundreds of thousands of plants but only sow the tiles somebody is looking at, and a town whose income depended on those would earn nothing while you were away and a fortune when you walked back. Any field that grows those two methods joins the economy without this file changing, which is where the next pass gets its bushes.
...
**Buildings are rows too, and their footprint is the interesting part.** Each kind is one `MultiMesh` — a purple cube for the cloner, an orange box for a hut — and a finished one blocks its 3×3 of grid cells, so from then on it steers walkers and it stops the next building from being placed on top of it, using the same passability the chasm uses. Completing one also fires a single area hit at its own middle, which clears the flora it was raised over and shoves any Meep standing inside it back onto open ground; both of those are things the world already knew how to do to a blast. The one thing it does not clear is the far-distance grass field, which ignores being cut the same way it ignores craters, so a town seen from a long way off still has a dozen blades standing where its walls are. Twelve `StaticBody3D` colliders are lent to whichever finished structures are within 90 m of a player, on exactly the same argument as the Meep pick proxies: you can only walk into the ones you can see, and a planet of towns cannot afford a body per shed. Placement spirals outward from the ship on a ring search and refuses ground that is not level to within a metre, which is why the first huts appear along the flat side of the landing and not in the chasm.

**The cloner is the town's only way to grow, and it is deliberately a queue.** Five Meeps fit inside at once with three more allowed to wait at the door; one goes in, twelve biomass goes with it, and a second later two come out. A Meep inside is not drawn and not walked but is still very much alive — it is the reason there is a state byte at all. Growth is capped at thirty-two settlers for now, and the planner will not clone at all while it cannot also afford the next building, because a town that spends everything on population and then cannot pay for a hut is a town of unemployed spheres.

**What a Meep does next is a choice between posted work, not a state machine.** The colony posts what it wants doing — cut that tree, raise that wall, use the cloner — with a priority and a limit on how many may take it, and an idle settler takes the best score it can see. That score is the priority converted into metres minus the metres it would have to walk, so a job worth twice as much is worth a hundred metres further, plus a few metres of jitter that are a property of the settler rather than of the moment — which is what stops six Meeps released in the same second from picking the same tree six times. It is why the ones already near a building site are the ones who finish it, and why nobody stands still: a Meep that finds nothing worth taking wanders inside the claim instead, which is also what a town looks like when it has cut everything nearby and is waiting on biomass. Roughly half the settlers are kept on mining however much building there is to do, because a crew of builders with an empty bank is worth less than a slow street.

**Nothing about deciding what to do is allowed to cost a frame.** The planner runs every two seconds rather than every tick, and the flow field a job needs is baked on `WorkerThreadPool`, because a Dijkstra fill across sixteen thousand cells is not something to do between two frames — and it would happen at the worst possible moment, the instant a building is finished and everybody wants somewhere new to be. Until the field arrives the Meeps walk straight at the target and slide along whatever they bump into, which is a slightly worse route rather than a stutter. Twelve fields are kept, the least recently wanted is dropped, and two bakes are in flight at most.

Roads, and refusing to build near mobs that have killed a settler, are the pass after this one. The grid already carries a road flag that halves what a cell costs to cross and `TerrainScars` already recolours ground for craters, so neither of those has to come back and retrofit a route or a renderer.
...
The headless half is the interesting one, because it asks the questions a screenshot cannot: that the grid finds the sea and the crevasse at the real landing site, that the claim reaches its 100 m across the open ground and is stopped by water on some bearings and by the chasm on others, that a thousand routes home cross neither, that after a simulated minute no settler is standing on water or a slope or has strayed out of town, and that none of them has drifted from the ground under it — the failure a crowd of spheres hides best, since a Meep whose height has come loose reads as one buried in a hillside rather than as an error. It runs the town against the height field `game/world.tscn` actually ships, which means unsettled: leaving the old town pads on would flatten the exact terrain the checks exist to meet, and did, the first time.

Then it lets the colony live and audits the result as an economy rather than as a set of features: that real trees came out of the world's own flower-tree field and stopped being offered as work; that biomass was earned for them and went into something standing; that the bank never promised units it did not hold; that the cloner went up first, that housing followed it, and that the population grew past its opening wave without running past its cap; that no two buildings were pegged out on the same ground, that a finished one took its whole footprint out of the grid, and that nobody was left standing inside the walls of it; and that a client fed nothing but the joiner snapshot rebuilds the same town in the same places. It also records how long each milestone took, because the way this system will break as it grows is by getting slower rather than by getting wrong. Run with a renderer it photographs the settlers at arm's length, the wall on its own, the cloner with a crew on it, the colony from the distance it is played at, and the town from above — and it asks the one question headless cannot, whether a finished building is standing on bare ground, because the cover fields only sow the tiles a viewer is near and a run with nobody in it has no grass near the town to clear. It asks that by trying to clear the footprint a second time and requiring that there is nothing there to cut, with the same volume fired next to the ship as the control: forty plants' worth of flora a few metres away, none at all inside the walls.
