...
Run from the repository root with Blender 5.1:
    ...
    & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" `
        ...
        --python blender_assets/source/build_vat_assets.py

The source .blend files are opened read-only and are never saved.  Runtime
outputs are written directly under ``blender_assets`` so the neighbouring
``source/.gdignore`` does not hide them from Godot.
"""
