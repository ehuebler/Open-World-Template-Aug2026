"""Turn `generated/character_3.blend` into the settler: a rigged, animated body
plus three garments cut out of `generated/dressed.blend`.

Run headless from the project root:

    & "C:\\Program Files\\Blender Foundation\\Blender 5.1\\blender.exe" --background --python blender_assets/source/build_character_3.py
...
ROOT = os.path.abspath(os.path.join(SOURCE_DIR, os.pardir, os.pardir))
ASSET_DIR = os.path.abspath(os.path.join(SOURCE_DIR, os.pardir))

SRC_BLEND = os.path.join(ROOT, "generated", "character_3.blend")
...
def export_garment(garment: bpy.types.Object, rig: bpy.types.Object) -> None:
    path = os.path.join(ASSET_DIR, garment.name + ".glb")
    if bpy.context.object is not None and bpy.context.object.mode != "OBJECT":
        bpy.ops.object.mode_set(mode="OBJECT")
