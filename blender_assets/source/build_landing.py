...
opens each source in turn:
    ...
    & $blender --background --factory-startup --python blender_assets/source/build_landing.py

It writes `blender_assets/colony_ship.glb` and `blender_assets/purple_flower.glb`,
and never saves the sources in `generated/`, so re-exporting either of them from
...
OUT_DIR = os.path.abspath(os.path.join(SOURCE_DIR, os.pardir))
...
def open_source(name):
    bpy.ops.wm.open_mainfile(filepath=os.path.join(PROJECT_DIR, "generated", name))
